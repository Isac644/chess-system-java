...
public class ChessMatch {
	...
	public Piece makeMove(Position source, Position target) {
		...
		if(piece instanceof King && (source.getColumn() - target.getColumn()) < -1) {
			Position rookS = new Position(source.getRow(), source.getColumn() + 3);
			Position rookT = new Position(source.getRow(), source.getColumn() + 1);
			Piece rook = board.removePiece(rookS);
			board.placePiece(rook, rookT);
			((ChessPiece)rook).increaseMoveCount();
		}
		
		if(piece instanceof King && (source.getColumn() - target.getColumn()) > 1) {
			Position rookS = new Position(source.getRow(), source.getColumn() - 4);
			Position rookT = new Position(source.getRow(), source.getColumn() - 1);
			Piece rook = board.removePiece(rookS);
			board.placePiece(rook, rookT);
			((ChessPiece)rook).increaseMoveCount();
	...
	public void undoMove(Position source, Position target, Piece capturedPiece) {
		...
		if(piece instanceof King && (source.getColumn() - target.getColumn()) < -1) {
			Position rookS = new Position(source.getRow(), source.getColumn() + 3);
			Position rookT = new Position(source.getRow(), source.getColumn() + 1);
			Piece rook = board.removePiece(rookT);
			board.placePiece(rook, rookS);
...
//		placeNewPiece('c', 5, new Bishop(board, Color.BLACK));
		...
		placeNewPiece('h', 5, new Queen(board, Color.BLACK));
